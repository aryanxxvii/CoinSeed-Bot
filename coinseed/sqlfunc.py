...
mydb = mysql.connector.connect(host="johnny.heliohost.org", user="aarav_root", passwd=passwd, database="aarav_db")
...
def sql_server_topusers(guid): #list the players in the server, csym
    ...
    mycursor.execute("SELECT DUID, CBAL FROM DUSERS WHERE GUID = {} ORDER BY CBAL DESC".format(int(guid)))
